...
from pipeline_common import parse_run_failure, run_failed
from validate_workflow_state import (
    AutoLabelingPhase, AutoLabelingState, LabelingBackend, LabelingPath,
    LastRun, WorkflowState,
)

# Real shape returned by a non-streaming run tool that reports a single
# summarized error line.
FAILED_RESULT = (
...
# Real shape returned by the streaming auto-labeling path in
# pipeline_handlers/auto_labeling.py.
...
def test_run_failed_fallback_without_sentinel_prefix():
    ...
    assert run_failed("Auto-labeling failed with exit code 2.\n") is True
...
def test_record_run_restarts_count_for_another_workflow():
    ...
    with patch.object(WorkflowState, "save"):
        state.record_run("auto_labeling", failed=True, error="a")
        state.record_run("msight_pipeline", failed=True, error="b")
