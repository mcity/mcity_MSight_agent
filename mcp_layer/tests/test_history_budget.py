...
def test_pipeline_reset_leaves_the_counter_at_zero():
    """The CONFIRMED reset ends the session, so the budget goes back to 0.
    reset_workflow_state is two-step (see tests/test_reset_guard.py), so the
    flag stands in for the user having already agreed."""
    pipeline = ChatPipeline(mcp_client=MagicMock(), llm=MagicMock())
    pipeline.state = _state(4)
    pipeline.state.reset_awaiting_confirmation = True
...
def test_unconfirmed_reset_keeps_the_counter():
    """The question alone must not shorten the history window: the session is
    still live, and the model needs those turns to read the user's answer."""
    pipeline = ChatPipeline(mcp_client=MagicMock(), llm=MagicMock())
    pipeline.state = _state(4)
    pipeline._user_texts = ["reset workflow"]
    client = MagicMock()
    ...
    assert pipeline.state.turns_since_reset == 4


def test_switching_workflow_keeps_the_counter():
    """A switch clears the parameters, not the conversation."""
    with patch.object(WorkflowState, "save"):
        fresh = _state(3).reset_for_workflow("msight_pipeline")
    assert fresh.turns_since_reset == 3
    assert fresh.workflow_name == "msight_pipeline"
