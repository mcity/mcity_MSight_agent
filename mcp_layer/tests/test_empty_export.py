...
so it must not lock the workflow. Two entry points are covered:
  ...
  - the manual path (export_to_cvat / export_to_label_studio called directly):
    no task id is recorded and the phase does not advance
...
from pipeline_common import HardStop, export_empty
...
GOOD_CVAT = (
    "Dataset 'custom_dataset2' uploaded to CVAT successfully.\n"
    "Task ID: 2474952\n"
    "Images: 30\n"
    "Open in CVAT: https://app.cvat.ai/tasks/2474952"
)
...
def test_export_empty_detects_zero_image_count():
    ...
    assert export_empty("Task ID: 5\nImages: 0\nOpen in CVAT: url") is True


def test_export_empty_detects_padded_zero_image_count():
    """Label Studio pads the summary column; CVAT does not."""
    assert export_empty("Project ID : 3\nImages     : 0\nTasks      : 0\n") is True
...
def test_export_empty_does_not_match_another_zero_count_line():
    assert export_empty("Images: 10\nSkipped Images: 0\n") is False
...
def test_empty_cvat_export_records_no_task_id():
    pipeline, _ = _run_manual_export(EMPTY_CVAT)
    assert not pipeline.state.auto_labeling.cvat_task_id
...
def test_good_cvat_export_still_records_task_and_locks():
    pipeline, routings = _run_manual_export(GOOD_CVAT)
    assert pipeline.state.auto_labeling.cvat_task_id == 2474952
...
def test_good_export_after_run_locks_and_records_success():
    ...
    assert al.cvat_task_id == 2474952
