...
def test_locked_reply_names_no_tool():
    ...
    assert "switch_workflow" not in routings[0].reply
    assert "(" not in routings[0].reply
    assert "locked" in routings[0].reply
    # The model still receives the full instruction in the tool result. This
    # workflow restarts through switch_workflow(confirm_restart=true), so that
    # is the instruction to look for here.
    assert "confirm_restart=true" in result
